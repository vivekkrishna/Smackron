...
$decryptedID = base64_decode($_SESSION['uid']);
$id_array = explode("p3h9xfn8sq03hs2234", $decryptedID);
//$my_id = $id_array[1];
$my_id=$_SESSION['uid'];
$my_uname = $_SESSION['fullname']; // Put user's first name into a local variable
...
if (isset($_POST['deleteBtn'])) {
    foreach ($_POST as $key => $value) {
		...
		$sql = mysql_query("SELECT * FROM private_messages WHERE id='$value' LIMIT 1");

while($row = mysql_fetch_array($sql)){ 
    if($row['senderDelete'] == "1"){
	$sql = mysql_query("DELETE FROM private_messages WHERE id='$value'");	    
    } }
...
<body>
    <?php //include_once "header_template.php"; ?><center>
<table width="680" style="background-color:white;" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="690" valign="top">
  <h2 style="margin-left:24px;">Your Private Messages <?php echo $_SESSION['fullname']; ?></h2>
...
while($row = mysql_fetch_array($sql)){ 
    ...
    $ret = mysql_query("SELECT id, fullname FROM user WHERE id='$fr_id' LIMIT 1");
    while($raw = mysql_fetch_array($ret)){ $Sid = $raw['id']; $Sname = $raw['fullname']; }
...
?>
        <table width="96%" border="0" align="center" cellpadding="4">
          <tr>
            ...
            <td width="20%" valign="top"><a href="othersprof.php?uid=<?php echo $Sid; ?>"><?php echo $Sname; ?></a></td>
